  ...
  return (
    <Frame>
      ...
      <div className="px-5 pt-5 pb-8 border-b border-ink/7" style={{ background: 'rgba(255,255,255,0.7)' }}>
        ...
        <h1 className="font-display text-3xl font-bold text-ink mb-2" style={{ lineHeight: 1.05, letterSpacing: '-0.02em' }}>
          {s.name}
        </h1>
        <p className="text-sm text-ink/50 italic" style={{ lineHeight: 1.55 }}>{s.tagline}</p>
      </div>
      <div className="px-5 pt-6 pb-12">
            ...
            {s.founders.map((f) => (
              <div key={f.name} className="flex items-center gap-3">
                <div className="w-[38px] h-[38px] bg-orange flex items-center justify-center shrink-0">
                  <span className="font-display text-xs font-bold text-white">{initials(f.name)}</span>
                </div>
                <div>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div className="border border-ink/9 p-4" style={{ background: 'rgba(255,255,255,0.8)' }}>
              <p className="text-[10px] font-semibold text-muted uppercase mb-1.5" style={{ letterSpacing: '0.1em' }}>Ask</p>
              <p className="font-display text-[22px] font-bold text-ink leading-none" style={{ letterSpacing: '-0.02em' }}>{s.metrics.ask}</p>
        ...
        {r ? (
          <div className="border-2 border-orange p-6 text-center" style={{ background: 'rgba(255,255,255,0.7)' }}>
            <p className="text-[10px] font-semibold text-orange uppercase mb-2.5" style={{ letterSpacing: '0.14em' }}>Interest Locked</p>
            <p className="font-display text-5xl font-bold text-ink leading-none" style={{ letterSpacing: '-0.03em' }}>{r.score}/10</p>
          ...
          <Link to={`/rate/${s.id}`}
                className="block w-full text-center bg-orange text-white py-[18px] font-display font-bold text-base"
